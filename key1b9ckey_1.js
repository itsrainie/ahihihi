...
  // Check if the active element is not an input field or a textarea
  const activeElement = document.activeElement;
  const isTyping = activeElement && (activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA');

  // If not typing and not locked, process the aimbot key
  if (!isTyping && !espConfig.lockAimbotTriggerBot && e.key === espConfig.aimbotKey) {
    espConfig.aimbot = (espConfig.aimbot + 1) % 4;
    // Prevent default action if needed, though unlikely to interfere here
    // e.preventDefault();
  }
  // If not typing and not locked, process the triggerbot key
  if (!isTyping && !espConfig.lockAimbotTriggerBot && e.key === espConfig.triggerBotKey) {
    espConfig.triggerBot = (espConfig.triggerBot + 1) % 4;
    // Prevent default action if needed
    // e.preventDefault();
  }

  // Toggle UI key (this usually works even when typing, which is often desired)
  if (e.key === espConfig.toggleUIKey) {
    lilGuiContainer.style.display = lilGuiContainer.style.display === 'none' ? 'block' : 'none';
  }

  // Auto-claim ads key (adjust if this should also be blocked while typing)
  if (e.key === 'e') {
...
const colors = {
  ...
  player: new THREE.Color(0x0055de),
...
const crosshairMaterial = new THREE.LineBasicMaterial({ color: 0xe6e600, transparent: true, opacity: 0.5 });
